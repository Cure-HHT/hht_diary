...
export class RateLimiterService {
  ...
  private static readonly DEFAULT_WINDOW_DURATION = 60000; // 1 minute in milliseconds
   ...
   * @param key Unique identifier
   */
  private removeExpiredAttempts(key: string): void {
    ...
    record.timestamps = record.timestamps.filter(
      timestamp => this.isTimestampValid(timestamp, now)
    );
   ...
   */
  private isTimestampValid(timestamp: number, now: number = Date.now()): boolean {
   ...
   */
  private getCurrentAttemptCount(key: string): number {
    const record = this.attempts.get(key);
   ...
   */
  private getOldestAttemptTimestamp(key: string): number | null {
    const record = this.attempts.get(key);
