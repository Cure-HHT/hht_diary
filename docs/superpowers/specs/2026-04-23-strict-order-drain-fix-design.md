...
- REQ-d00119 (FIFO row schema)
- REQ-d00122 (Destination contract; subscription filter)
- REQ-d00123, REQ-d00124, REQ-d00127, REQ-d00131 (FIFO drain semantics, terminal status, wedge handling)
- REQ-d00132 (rehabilitate — superseded by REQ-d00144 tombstoneAndRefill)
- REQ-d00144 (tombstoneAndRefill atomic recovery)

Implements REQ-p01001 (Offline Event Queue with Automatic Synchronization).

Per-implementation `// Implements:` and per-test `// Verifies:` annotations required. Implementation plan: `docs/superpowers/plans/2026-04-21-mobile-event-sourcing-refactor/PLAN_PHASE4.7_strict_order_drain.md`.
