...
- [ ] **Confirm Phase 2 complete**: on the shared branch, `git log --oneline` should show Phase 2's squashed commit as HEAD (or immediately behind any Phase-2-feedback fixups that have been folded in).
- [ ] **Stay on shared branch** (no new branch creation).
...
- [ ] **Create TASK_FILE** with output and Phase 2 completion SHA.
...
- [ ] **Baseline**.
- [ ] **Create TASK_FILE**.
- [ ] **Write REQ-MAT** into `spec/dev-event-sourcing-mobile.md`.
- [ ] **Update `spec/INDEX.md`** with the new REQ row.
- [ ] **Commit**: "Add materializer contract assertions (CUR-1154)".
...
- [ ] **Baseline**: green.
- [ ] **Create TASK_FILE**.
- [ ] **Write failing tests** for the interface. Include a simple `MapEntryTypeDefinitionLookup` concrete for use by tests elsewhere in Phase 3:
...
- [ ] **Implement** the abstract class with a single method `EntryTypeDefinition? lookup(String entryTypeId);`. Plus the test-only `MapEntryTypeDefinitionLookup` exported from a `test/test_support/` directory (not from `lib/`).
- [ ] **Run tests**; expect pass.
- [ ] **Lint**: clean.
- [ ] **Commit**: "Add EntryTypeDefinitionLookup interface (CUR-1154)".
...
- [ ] **Baseline**: green.
- [ ] **Create TASK_FILE**.
- [ ] **Write failing tests** (`materializer_test.dart`). Target signature:
...
- [ ] **Run tests**; expect failures for undefined `Materializer`.
...
- [ ] **Run tests**; expect pass.
- [ ] **Lint**: clean.
- [ ] **Commit**: "Implement Materializer.apply (CUR-1154)".
...
- [ ] **Baseline**: green.
- [ ] **Create TASK_FILE**.
- [ ] **Write failing tests** using `SembastBackend` with an in-memory database and `MapEntryTypeDefinitionLookup` (from Task 3):
...
- [ ] **Run tests**; expect failures.
- [ ] **Implement `rebuildMaterializedView(StorageBackend backend, EntryTypeDefinitionLookup lookup)`**:
...
- [ ] **Run tests**; expect pass.
- [ ] **Lint**: clean.
- [ ] **Commit**: "Implement rebuildMaterializedView (CUR-1154)".
...
- [ ] **Baseline**: green.
- [ ] **Create TASK_FILE**.
- [ ] **Export** `Materializer`, `EntryTypeDefinitionLookup`, `rebuildMaterializedView` from the top-level barrel.
- [ ] **Keep `MapEntryTypeDefinitionLookup` out of `lib/`**. It lives under `test/test_support/` and is imported via relative path by tests. (Reason: it's a test double; we don't want apps to depend on it.)
- [ ] **Commit**: "Export materialization public surface (CUR-1154)".
...
- [ ] **Bump `append_only_datastore` version** (minor bump).
- [ ] **Update `CHANGELOG.md`** with bullets for Materializer, rebuildMaterializedView, EntryTypeDefinitionLookup.
- [ ] **Full verification**:
  - `(cd apps/common-dart/append_only_datastore && flutter test && flutter analyze)`
  - `(cd apps/daily-diary/clinical_diary && flutter test && flutter analyze)` (no change expected)
- [ ] **Commit**: "Bump append_only_datastore for Phase 3 (CUR-1154)".
...
- [ ] **Rebase onto main** in case main moved: `git fetch origin main && git rebase origin/main`. Phases 1 and 2's squashed commits remain as-is.
- [ ] **Full verification**: `flutter test` / `flutter analyze` across all touched packages.
- [ ] **Interactive rebase to squash Phase 3 commits**: `git rebase -i origin/main` — keep Phase 1 and Phase 2 commits as `pick`, squash all Phase 3 commits into one with message:
...
- [ ] **Comment on PR**: "Phase 3 ready for review — commit `<sha>`. Review focus: materializer correctness across the three event types, `effective_date_path` fallback behavior, rebuild idempotence. Materializer is still unwired."
- [ ] **Wait for phase review**. Address feedback via fixups + in-place rebase.
- [ ] **Record phase-completion SHA** in TASK_FILE before starting Phase 4.
