...
The Evidence Records system addresses these requirements through independent, cryptographically-verified proof rather than relying solely on internal system timestamps. Each participant diary entry is anchored to a tamper-proof record that can be independently verified during regulatory audits.
...
- **Who**: Participant identity verification linking data to a specific enrolled participant
...
### Participant-Friendly Authentication

The system leverages the security participants already use daily—their device's built-in authentication (fingerprint, face recognition, or PIN). This approach:
...
Participant identity verification uses one-way cryptographic hashing. The system stores proof of identity without storing actual personal information. During an audit, the Sponsor provides the original email to the auditor, who can independently verify it matches the stored hash and contact the participant directly if needed.
...
- **Attributable**: Each entry is linked to a verified participant identity
...
- Participant enters diary data on their device
...
**Backward Traceability** (Audit → Participant)
- Auditor receives evidence record with cryptographic proofs
- Timestamps verified against independent public records
- Device consistency verified across participant's submissions
- If needed, auditor can contact participant directly via Sponsor-provided email
- Participant can confirm their participation and data authenticity
...
**What if a participant doesn't have a password set on their device?**

The system detects whether the device has a lock screen enabled. If not, the participant is prompted to set an in-app PIN before entering data. This ensures authentication is always in place, regardless of the participant's device settings.

**What happens if a participant loses their device or gets a new one?**

Device fingerprints are recorded with each submission, so auditors can see when a device change occurred. The participant re-enrolls on the new device, and the audit trail clearly shows the transition. Historical data remains linked to the original device fingerprint, maintaining the integrity of the evidence chain.

**Can site coordinators access participant PINs?**

No. Site coordinators can send PIN reset notifications to participants, but they cannot view or retrieve participant PINs. This separation ensures that authentication credentials remain under participant control.
...
**What if a participant denies entering data during an audit?**

The evidence record provides multiple layers of verification: the device fingerprint confirms which device was used, the authentication status confirms privileged access was verified, and the hashed email links to the enrolled participant. The auditor can contact the participant directly using the Sponsor-provided email to resolve any discrepancies.

**Is participant location always tracked?**

No. Geolocation is disabled by default and only collected when explicitly enabled by the Sponsor for a specific trial. When enabled, participants are clearly informed that location data is being collected, and they can deny location permissions without affecting their ability to enter diary data.
