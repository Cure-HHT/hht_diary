/**
 * Spec Review Position Highlighting Module
 *
...
(function(RS) {
    ...
    class ResolvedPosition {
         ...
         */
        isExact() {
            return this.confidence === RS.Confidence.EXACT;
        }
         ...
         */
        isApproximate() {
            return this.confidence === RS.Confidence.APPROXIMATE;
        }
         ...
         */
        isUnanchored() {
            return this.confidence === RS.Confidence.UNANCHORED;
        }
        ...
        getHighlightClass() {
            switch (this.confidence) {
                case RS.Confidence.EXACT: return RS.HighlightClass.EXACT;
                case RS.Confidence.APPROXIMATE: return RS.HighlightClass.APPROXIMATE;
                default: return RS.HighlightClass.UNANCHORED;
            }
        }
    }
    RS.ResolvedPosition = ResolvedPosition;
    ...
    RS.getTotalLines = getTotalLines;
    ...
    RS.getLineNumberFromCharOffset = getLineNumberFromCharOffset;
    ...
    RS.getLineCharRange = getLineCharRange;
    ...
    RS.findLineInText = findLineInText;
    ...
    RS.findContextInText = findContextInText;
    ...
    RS.findKeywordOccurrence = findKeywordOccurrence;
    ...
    RS.resolvePosition = resolvePosition;
            ...
            case RS.PositionType.LINE: {
                if (position.lineNumber > totalLines) {
                    return new ResolvedPosition({
                        ...
                        confidence: RS.Confidence.UNANCHORED,
                ...
                return new ResolvedPosition({
                    originalPosition: position,
                    confidence: RS.Confidence.EXACT,
    ...
    function resolveWithFallback(position, content) {
        ...
        if (lineToTry !== null && lineToTry <= totalLines) {
            ...
            return new ResolvedPosition({
                originalPosition: position,
                confidence: RS.Confidence.APPROXIMATE,
    ...
    RS.highlightPosition = highlightPosition;
    ...
    function clearHighlights(element) {
        ...
        const highlighted = element.querySelectorAll(
            `.${RS.HighlightClass.EXACT}, .${RS.HighlightClass.APPROXIMATE}, ` +
            `.${RS.HighlightClass.UNANCHORED}, .${RS.HighlightClass.ACTIVE}`
        );
        highlighted.forEach(el => {
            el.classList.remove(
                RS.HighlightClass.EXACT,
                RS.HighlightClass.APPROXIMATE,
                RS.HighlightClass.UNANCHORED,
                RS.HighlightClass.ACTIVE
            );
        });
    }
    RS.clearHighlights = clearHighlights;
    ...
    function activateHighlight(threadId) {
        ...
        document.querySelectorAll('.' + RS.HighlightClass.ACTIVE).forEach(el => {
            el.classList.remove(RS.HighlightClass.ACTIVE);
        });

        // Activate matching
        if (threadId) {
            document.querySelectorAll(`[data-thread-id="${threadId}"]`).forEach(el => {
                el.classList.add(RS.HighlightClass.ACTIVE);
            });
        }
    }
    RS.activateHighlight = activateHighlight;
