...
2. Test Linear integration:
   ```bash
   node tools/anspar-marketplace/plugins/linear-integration/scripts/test-config.js
   ```

3. Manual ticket creation:
   ```bash
   python3 scripts/detect-changes.py --format json | jq '.changed_requirements[0]' | \
     node tools/anspar-marketplace/plugins/linear-integration/scripts/create-verification-ticket.js
   ```
