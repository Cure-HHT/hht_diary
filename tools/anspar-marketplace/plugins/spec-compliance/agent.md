...
description: Use this agent proactively whenever files in the spec/ directory are created, modified, or about to be committed. This agent should be triggered automatically through git hooks (pre-commit) to enforce spec/README.md guidelines and the /command remove-prd-code.md process.\n\nExamples:\n- <example>\n  Context: User has just edited spec/prd-app.md adding code examples\n  user: "I've updated the PRD to include some implementation examples"\n  assistant: "Let me use the spec-compliance-enforcer agent to review these changes for compliance with spec/ guidelines."\n  <commentary>The spec-compliance-enforcer agent should check that PRD files don't contain code examples per spec/README.md rules</commentary>\n  </example>\n- <example>\n  Context: User is about to commit changes to spec/ops-deployment.md\n  assistant: "Before committing, I'm going to use the spec-compliance-enforcer agent to validate spec/ directory compliance."\n  <commentary>Proactively enforce spec/ guidelines before git commits</commentary>\n  </example>\n- <example>\n  Context: User creates a new file spec/dev-api-implementation.md with vague SHALL statements\n  user: "I've added the new API spec"\n  assistant: "I'll use the spec-compliance-enforcer agent to ensure this follows the requirements format and is properly scoped."\n  <commentary>Validate new spec files follow hierarchical naming, requirement format, and audience scope rules</commentary>\n  </example>\n- <example>\n  Context: Pre-commit hook is triggered\n  assistant: "Running spec-compliance-enforcer agent to validate spec/ changes before commit."\n  <commentary>Automatic validation through git hooks</commentary>\n  </example>
model: sonnet
color: green
---
