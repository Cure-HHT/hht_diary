...
(function(RS) {
    ...
    RS.ApprovalDecision = Object.freeze({
        APPROVE: 'approve',
        REJECT: 'reject'
    });

    RS.VALID_REQ_STATUSES = ['Draft', 'Active', 'Deprecated'];

    RS.DEFAULT_APPROVAL_RULES = {
     ...
     */
    RS.validateReqId = function(reqId) {
     ...
     */
    RS.normalizeReqId = function(reqId) {
        if (!reqId) return '';
    ...
    class CommentPosition {
        ...
        validate() {
            const errors = [];
            const validTypes = Object.values(RS.PositionType);

            if (!validTypes.includes(this.type)) {
                errors.push(`Invalid position type: ${this.type}`);
            }

            if (!RS.validateHash(this.hashWhenCreated)) {
                errors.push(`Invalid hash format: ${this.hashWhenCreated}`);
            }

            if (this.type === RS.PositionType.LINE) {
            ...
            if (this.type === RS.PositionType.BLOCK) {
            ...
            if (this.type === RS.PositionType.WORD) {
                if (!this.keyword) {
    ...
    RS.CommentPosition = CommentPosition;
    ...
    class Thread {
        ...
        static create(reqId, creator, position, initialComment) {
            const thread = new Thread({
                threadId: RS.generateUuid(),
                reqId: reqId,
                createdBy: creator,
                createdAt: RS.nowIso(),
                position: position,
        ...
        validate() {
            ...
            if (!RS.validateReqId(this.reqId)) {
    ...
    class Approval {
        ...
        validate() {
            ...
            if (!Object.values(RS.ApprovalDecision).includes(this.decision)) {
    ...
    class StatusRequest {
        constructor(data) {
            ...
            this.state = data.state || RS.RequestState.PENDING;
        }

        static create(reqId, fromStatus, toStatus, requestedBy, justification, requiredApprovers) {
            if (!requiredApprovers) {
                const key = `${fromStatus}->${toStatus}`;
                requiredApprovers = RS.DEFAULT_APPROVAL_RULES[key] || ['product_owner'];
            }
            return new StatusRequest({
                requestId: RS.generateUuid(),
                reqId: reqId,
                type: 'status_change',
                fromStatus: fromStatus,
                toStatus: toStatus,
                requestedBy: requestedBy,
                requestedAt: RS.nowIso(),
                justification: justification,
                approvals: [],
                requiredApprovers: requiredApprovers,
                state: RS.RequestState.PENDING
            });
        }
        ...
        _updateState() {
            if (this.state === RS.RequestState.APPLIED) return;

            // Check for rejections
            for (const approval of this.approvals) {
                if (approval.decision === RS.ApprovalDecision.REJECT) {
                    this.state = RS.RequestState.REJECTED;
                    return;
                }
            }
            ...
            const approvedUsers = new Set(
                this.approvals
                    .filter(a => a.decision === RS.ApprovalDecision.APPROVE)
                    .map(a => a.user)
            );
            ...
            this.state = allApproved ? RS.RequestState.APPROVED : RS.RequestState.PENDING;
        }

        markApplied() {
            if (this.state !== RS.RequestState.APPROVED) {
                throw new Error('Can only apply approved requests');
            }
            this.state = RS.RequestState.APPLIED;
        }
        ...
        validate() {
            ...
            if (!RS.VALID_REQ_STATUSES.includes(this.fromStatus)) {
                errors.push(`Invalid fromStatus: ${this.fromStatus}`);
            }
            if (!RS.VALID_REQ_STATUSES.includes(this.toStatus)) {
            ...
            if (!Object.values(RS.RequestState).includes(this.state)) {
    ...
    RS.StatusRequest = StatusRequest;
    ...
    RS.ReviewSession = ReviewSession;
    ...
    class ReviewConfig {
        constructor(data) {
            this.approvalRules = data.approvalRules || Object.assign({}, RS.DEFAULT_APPROVAL_RULES);
    ...
    class ReviewState {
         ...
         */
        getThreads(reqId) {
            const normalizedId = RS.normalizeReqId(reqId);
         ...
         */
        addThread(thread) {
            const normalizedId = RS.normalizeReqId(thread.reqId);
         ...
         */
        addRequest(request) {
            const normalizedId = RS.normalizeReqId(request.reqId);
