...
(function() {
    ...
    async function fetchPackages() {
        try {
            ...
            console.log('Packages loaded:', {
                count: RS.packages.items.length,
                activeId: RS.packages.activeId,
                packages: RS.packages.items.map(p => ({
                    id: p.packageId,
                    name: p.name,
    ...
    function applyPackageContext() {
        ...
        updateContextIndicator(RS.packages.activeId);
    ...
    function togglePackageFilter(event) {
        ...
        RS.packages.filterEnabled = !RS.packages.filterEnabled;

        // Update toggle button styling
        const toggle = document.getElementById('packageFilterToggle');
        if (toggle) {
            toggle.classList.toggle('active', RS.packages.filterEnabled);
        }
    ...
    RS.togglePackageFilter = togglePackageFilter;
