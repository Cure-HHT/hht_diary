...
class HTMLGenerator:
    ...
    def _load_review_js(self) -> str:
        ...
        module_order = [
            ...
            "review-packages.js",  # Package management (depends on sync)
            "review-comments.js",  # Comments UI (depends on position, sync)
            "review-status.js",    # Status UI (depends on sync)
    ...
    <script>
        ...
        function applyFilters() {
            ...
            document.querySelectorAll('.req-item').forEach(item => {
                ...
                // Show/hide with hierarchy promotion for filtered items
                if (matches) {
                    item.classList.remove('filtered-out');
                    // If any filter is active, promote matching items through hierarchy
                    if (anyFilterActive) {
                        item.classList.remove('collapsed-by-parent');
