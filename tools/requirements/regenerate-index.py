...
def parse_requirements_from_file(file_path: Path) -> List[Tuple[str, str, str, str]]:
    ...
    for header_match in req_pattern.finditer(content):
        ...
        if not status_match:
            print(f"⚠️  {file_path.name}: REQ-{req_id} missing status line, skipping")
            continue

        # Find end marker
        end_marker_match = end_pattern.search(remaining[status_match.end():])
        if not end_marker_match:
            print(f"⚠️  {file_path.name}: REQ-{req_id} missing end marker, skipping")
            continue
...
'''
    ...
    for spec_file in sorted(spec_dir.glob('*.md')):
        ...
        if reqs:
            print(f"  ✓ {spec_file.name}: {len(reqs)} requirements")
    ...
    print(f"{'='*60}")
    print(f"✅ INDEX.md regenerated with {len(all_reqs)} requirements")
