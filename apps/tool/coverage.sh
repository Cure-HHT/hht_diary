#!/bin/bash
# Rollup coverage for ALL projects across the entire apps directory
...
for project in "${PROJECTS[@]}"; do
    ...
    if [ -x "$TEST_SCRIPT" ]; then
        ...
        if [ -f "$CHILD_LCOV" ]; then
            REWRITTEN="$SCRIPT_DIR/coverage/lcov-${SLUG}.info"
            sed "s|^SF:|SF:apps/$project/|" "$CHILD_LCOV" > "$REWRITTEN"
...
echo ""
echo "=============================================="
echo "All Apps Coverage Summary"
echo "=============================================="

for project in "${PROJECTS[@]}"; do
    SLUG="${project//\//-}"
    CHILD_LCOV="$SCRIPT_DIR/coverage/lcov-${SLUG}.info"
    if [ -f "$CHILD_LCOV" ]; then
...
if [ -f "$SCRIPT_DIR/coverage/lcov.info" ]; then
    ...
    printf "  %-40s %s%%\n" "COMBINED" "$TOTAL_PCT"
    echo ""
    echo "  Report: apps/coverage/lcov.info"
    [ -f "$SCRIPT_DIR/coverage/html/index.html" ] && echo "  HTML:   apps/coverage/html/index.html"
fi
