...
void main() {
  ...
  test(
      'admin effective permissions include assign_site + '
      'portal.user.view_accounts (drives PermissionGate)', () async {
    final db =
    ...
    final bootstrap = await buildPortalAuthorizationPolicy(
      ...
      roleGrantsYaml: loadRolePermissionsYaml(
    ...
    expect(names, contains('portal.user.view_accounts'),
        reason: 'admin can subscribe to the user-accounts views');

    // The coordinator's snapshot must NOT carry those, so its PermissionGates
    // close (no Assign widget; "no access" for the user list).
    final coordinator = Principal.user(
      userId: 'sc-1',
      roles: const {'StudyCoordinator'},
      activeRole: 'StudyCoordinator',
    ...
    expect(scNames, isNot(contains('portal.user.view_accounts')));
  });

  test(
      'SystemOperator effective permissions include the user-provisioning set: '
      'portal.user.view_accounts + portal.site.view + portal.user.create '
  ...
  test(
      'role->permission grants survive a reboot on an already-seeded store: a '
      'SECOND boot re-affirms the grants (deployed envs pick up new grants on '
      'redeploy, no DB reset)', () async {
    // Simulates the deploy path: the store is already seeded (marker present),
    // so the seed-once gate is skipped — the role->permission grants now come
    // from the validated action-permission seed (bootstrapActionPermissions),
    // whose idempotency is the framework's own concern, so this test only
    // verifies the grants are STILL EFFECTIVE after a reboot.
    ...
    // The grant is still effective after the reboot.
    final bootstrap = await buildPortalAuthorizationPolicy(
      eventStore: boot2.eventStore,
      roleGrantsYaml: loadRolePermissionsYaml(
        resolveSponsorConfigDir(const <String, String>{}),
      ),
    );
    ...
    expect(
        names,
        containsAll(
            <String>['portal.user.view_accounts', 'portal.user.create']));
  });
  ...
  test(
    ...
    // Administrator's effective read permissions, under the CORRECTED matrix:
    // it holds site.view, rave.view_sync and user.view_accounts, but NOT
    // participant.view or questionnaire.view_status (the CUR-1474 corrections).
    final bootstrap = await buildPortalAuthorizationPolicy(
      eventStore: boot.eventStore,
      roleGrantsYaml: loadRolePermissionsYaml(
        resolveSponsorConfigDir(const <String, String>{}),
    ...
    expect(
      ...
      reason: 'admin granted the operational read permissions',
    );
    // CUR-1474 corrections: Administrator no longer holds these reads.
    expect(names, isNot(contains('portal.participant.view')),
        reason: 'admin dropped from View Participant');
    expect(names, isNot(contains('portal.questionnaire.view_status')),
        reason: 'admin dropped from View Questionnaire Status');
  });

  test(
      'GET /audit: admin (portal.audit.view) -> 200 with {rows, count}; '
      'sysop (no portal.audit.view) -> 403', () async {
    final db =
    ...
    // Administrator holds portal.audit.view (per role-permissions.yaml) -> 200.
    ...
    // SystemOperator lacks portal.audit.view (the corrected matrix grants audit
    // to SC + CRA + Admin, NOT the operator) -> 403 from our gate.
    // Bare userId: the dev validator resolves roles from user_role_scopes.
  ...
  test('CUR-1474: reads gate on Action permissions; no view:* grant survives',
      () async {
    expect(portalViewPermissionNamer('participant_record'),
        'portal.participant.view');
    final db = await newDatabaseFactoryMemory().openDatabase('namer-wiring.db');
    final boot = await bootstrapPortalServer(
      backend: SembastBackend(database: db),
      raveClient: DevSeedRaveClient(),
    );
    ...
    final names = (await policy.effectivePermissionsFor(admin))
        .rolePermissions
        .map((p) => p.name)
        .toSet();
    expect(names.any((n) => n.startsWith('view:')), isFalse,
        reason: 'view:* grants are retired; reads gate on Action permissions');
  });
}
