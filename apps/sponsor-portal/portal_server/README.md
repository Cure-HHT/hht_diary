...
HTTP server for the HHT Portal API, built on shelf/shelf_router.
...
All routes are defined in `routes.dart` and delegate to handlers from `portal_functions`:
...
Tests run automatically in GitHub Actions. See `.github/workflows/portal-server-ci.yml` for the full configuration.
...
Database and JWT configuration is handled by `portal_functions`. See that package's README for database environment variables.
