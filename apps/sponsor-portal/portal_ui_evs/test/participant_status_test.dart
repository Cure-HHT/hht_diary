...
void main() {
  test('entryType maps to status', () {
    expect(statusFromEntryType('participant_synced_from_edc'),
        ParticipantStatus.notConnected);
    expect(statusFromEntryType('participant_linking_code_issued'),
        ParticipantStatus.pending);
    expect(
        statusFromEntryType('participant_linked'), ParticipantStatus.connected);
    expect(statusFromEntryType('participant_trial_started'),
        ParticipantStatus.trialActive);
    expect(statusFromEntryType('participant_disconnected'),
        ParticipantStatus.disconnected);
    expect(statusFromEntryType('participant_reconnected'),
        ParticipantStatus.pending);
    expect(statusFromEntryType('participant_marked_not_participating'),
        ParticipantStatus.notParticipating);
    expect(statusFromEntryType('participant_reactivated'),
        ParticipantStatus.pending);
  ...
  test('enabledActions per state', () {
    ...
    expect(enabledActions(ParticipantStatus.trialActive),
        {ParticipantAction.disconnect, ParticipantAction.showCode});
