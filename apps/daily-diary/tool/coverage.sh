#!/bin/bash
# Rollup coverage for all daily-diary projects
...
echo "=============================================="
echo "Daily Diary - Rollup Coverage"
echo "=============================================="
...
for project in diary_functions diary_server clinical_diary; do
    ...
    if [ -x "$TEST_SCRIPT" ]; then
        ...
        if [ -f "$CHILD_LCOV" ]; then
            ...
            sed "s|^SF:|SF:apps/daily-diary/$project/|" "$CHILD_LCOV" > "$REWRITTEN"
...
if [ -f "$SCRIPT_DIR/coverage/lcov.info" ]; then
    ...
    printf "  %-25s %s%%\n" "COMBINED" "$TOTAL_PCT"
    echo ""
    echo "  Report: apps/daily-diary/coverage/lcov.info"
    [ -f "$SCRIPT_DIR/coverage/html/index.html" ] && echo "  HTML:   apps/daily-diary/coverage/html/index.html"
fi
