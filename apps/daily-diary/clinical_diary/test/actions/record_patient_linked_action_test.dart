...
// Verifies: DIARY-DEV-shared-events-catalog/A (patient_linked, P4)
import 'package:clinical_diary/actions/record_patient_linked_action.dart';
...
void main() {
  const action = RecordPatientLinkedAction();

  test('emits a finalized patient_linked on the Patient aggregate', () async {
    ...
    expect(draft.aggregateType, 'Patient');
    expect(draft.aggregateId, 'U-1'); // keyed on the stable user id
    expect(draft.entryType, 'patient_linked');
    expect(draft.eventType, 'finalized');
    expect(draft.data['patient_id'], 'P-42');
