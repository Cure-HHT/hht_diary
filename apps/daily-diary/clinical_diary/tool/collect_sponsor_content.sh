...
#
# Collect sponsor content from sponsor repositories into sponsor_content/
# for Docker build consumption.
#
# Usage:
#   tools/build/collect-sponsor_content.sh           # Clone from GitHub (CI/CD)
#
# Requires:
#   - jq (JSON parsing)
...
CONFIG_FILE="$(cd "$SCRIPT_DIR/../../../.." && pwd)/tools/build/sponsor-repos.json"
OUTPUT_DIR="$REPO_ROOT/assets/sponsor-content"
LOCAL_MODE=false
...
for i in $(seq 0 $((SPONSOR_COUNT - 1))); do
    ...
    if [ "$LOCAL_MODE" = true ]; then
        ...
        cp  "$CLONE_DIR/content/mobile/assets/images/status_badge.png" "$SPONSOR_OUTPUT"
        cp "$CLONE_DIR/content/sponsor-config.json" "$SPONSOR_OUTPUT"
