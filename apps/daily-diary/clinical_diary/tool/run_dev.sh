...
#
# Examples:
#   ./tool/run_dev.sh                                           # Run on default device
#   ./tool/run_dev.sh --web                                     # Run on Chrome
#   ./tool/run_dev.sh --import-file ./test/data/export.json     # Run with test data
#   ./tool/run_dev.sh --device macos --import-file data.json    # Run on macOS with data
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: ./tool/run_dev.sh [--import-file <path>] [--device <device>] [--web]"
            exit 1
            ;;
    esac
done
