...
void main() {
  ...
  final runOnMobileDevice = Platform.isAndroid || Platform.isIOS;
  ...
  group('mobile QA smoke automation', () {
    testWidgets('SMK-001 launches to an interactive screen', (tester) async {
      ...
      await _takeScreenshotIfAvailable(binding, 'SMK-001_launch');
    }, skip: !runOnMobileDevice);

    testWidgets(
      'SMK-003 reaches dashboard or enrollment entry point',
      (tester) async {
        await _launchAndPrepare(tester);
        ...
        await _takeScreenshotIfAvailable(binding, 'SMK-003_home_or_enrollment');
      },
      skip: !runOnMobileDevice,
    );
    ...
    testWidgets('A11Y-001 core controls expose semantic labels', (
      ...
      await _takeScreenshotIfAvailable(binding, 'A11Y-001_semantics');
    }, skip: !runOnMobileDevice);
    ...
    testWidgets('PERF-003 rapid core navigation does not crash', (
      ...
      await _takeScreenshotIfAvailable(binding, 'PERF-003_navigation_stress');
    }, skip: !runOnMobileDevice);
  });
}
...
Future<void> _takeScreenshotIfAvailable(
  ...
  try {
    ...
    debugPrint('Skipping screenshot $name: $error');
  }
}
