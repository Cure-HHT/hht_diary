...
> 
> **Constitutional Status**: These practices contain IMMUTABLE principles (marked 🔒) that require formal amendment process to change, and FLEXIBLE guidelines that can evolve with project needs.
...
## 🔒 Constitutional Principles for AI-Assisted Development
...
1. **🔒 AI-Generated Code Validation (NON-NEGOTIABLE)**
...
3. **Library-First Decision Making for AI**
   - If ticket specifies "library" or "major feature" → Use library-first
   - If ticket says "tweak", "fix", "adjust UI", "config" → Direct implementation OK
...
✅ Append-only (no updates or deletes to audit log)
✅ Immutable (tamper-evident with cryptographic hashing)
✅ Synchronized (captured at time of action, not batched)
✅ Complete (captures all required metadata)
✅ Retrievable (queryable for audits and investigations)
...
❌ **Never** disable audit trail capture for any reason
❌ **Never** modify historical audit records
❌ **Never** share authentication credentials
❌ **Never** store passwords in plain text
❌ **Never** skip validation testing "to save time"
❌ **Never** use production data in test environments
❌ **Never** make undocumented changes to validated systems
❌ **Never** assume compliance - verify and document
❌ **Never** implement compliance features without understanding requirements
❌ **Never** delete data that may be subject to retention requirements
❌ **Never** conflate audit trails (compliance) with operational logs (debugging)
...
1. **🔒 Structured Logging (NON-NEGOTIABLE)**
