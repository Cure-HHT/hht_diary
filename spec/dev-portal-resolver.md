...
B. The *Diary* app SHALL hold no static *Sponsor*-prefix-to-hostname mapping; the only sponsor-routing input bundled into the public build is the per-environment pointer to the neutral discovery endpoint (resolved per assertion D, not a compile-time constant).
...
*End* *Neutral Diary-Portal Resolution* | **Hash**: a2ecb651
