...
def handle_budget_alert(request):
    ...
    try:
        ...
        if "alertThresholdExceeded" not in billing_alert:
            print("No alertThresholdExceeded – periodic cost update, skipping Slack")
            return ("OK", 200)
        ...
        threshold_pct = float(billing_alert["alertThresholdExceeded"]) * 100
        # print(
        #     "Decoded billing alert summary: "
        #     f"budget_display_name={budget_display_name}, "
        #     f"cost_amount={cost_amount:.2f}, "
        #     f"budget_amount={budget_amount:.2f}, "
        #     f"threshold_pct={threshold_pct:.0f}"
        # )
        slack_message = (
            f"*Billing Alert for {budget_display_name}:* "
            f"Spend is *{cost_amount:.2f} USD* "
            f"(>{threshold_pct:.0f}% of budget *{budget_amount:.2f} USD*)."
        )
        # print(f"Constructed Slack message: {slack_message}")
        if SLACK_WEBHOOK_URL:
            ...
            print(
                f"SLACK_WEBHOOK_URL not set – skipping "
