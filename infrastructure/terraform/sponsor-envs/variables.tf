...
# -----------------------------------------------------------------------------
# Optional: Schema File Upload Configuration
# -----------------------------------------------------------------------------
#
# These variables control uploading database schema and sponsor data files
# to GCS for use by the db-schema-job Cloud Run job.
#
# IMPLEMENTS REQUIREMENTS:
...
variable "sponsor_data_file_name" {
  description = "Name of the sponsor data file in GCS (e.g., seed_callisto4_dev.sql)"
  type        = string
  default     = ""
}
...
variable "enable_cloud_run" {
  description = "Enable Cloud Run services (diary-server, portal-server)"
  type        = bool
  default     = false
}
