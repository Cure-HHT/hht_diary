...
if [ -f "$CLAUDE_PROJECT_DIR/apps/daily-dairy/clinical_diary/pubspec.yaml" ]; then
  echo "Installing Flutter dependencies for clinical_diary..."
  cd "$CLAUDE_PROJECT_DIR/apps/daily-dairy/clinical_diary"
  "$FLUTTER_DIR/bin/flutter" pub get
fi

# Install Node.js dependencies for Firebase Functions
if [ -f "$CLAUDE_PROJECT_DIR/apps/daily-dairy/clinical_diary/functions/package.json" ]; then
  echo "Installing Node.js dependencies for Firebase Functions..."
  cd "$CLAUDE_PROJECT_DIR/apps/daily-dairy/clinical_diary/functions"
  npm install
fi
